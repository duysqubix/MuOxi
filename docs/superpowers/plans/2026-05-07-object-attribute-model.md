...
> ## ✅ STATUS: COMPLETE (commits `e4d089c`..`53e4165` on master)
>
> All 11 tasks shipped. `cargo test -p db --features db-sqlite` → 5 passing
> (`account_roundtrip` + 4 new). `cargo check --workspace` green; Postgres
> backend also type-checks. Skip this plan when resuming — it's done.
>
> **One delta from the original plan worth knowing:** Task 10's test harness
> needed a comment-aware splitter. The naive `split(';')` works on Plan 1's
> migration (no comments) but breaks on the new objects migration (some SQL
> comments contain `;`, e.g. `-- own type_keys; the framework...`). Fix:
> strip `--` line comments from the FULL SQL first, then split. See
> `db/tests/integration_objects.rs::apply_schema`.
